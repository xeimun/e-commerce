...
| Method | Path | Description |
| --- | --- | --- |
| POST | `/api/v1/demo/reset` | 포트폴리오 데모 데이터 초기화 |
...
데모 API는 포트폴리오 시연 환경에서만 사용한다.
운영 환경에서는 비활성화하거나 접근을 제한한다.
...
- 데모 고객 ID `1`의 장바구니, 주문, 보유 쿠폰 상태를 재현 가능한 초기 상태로 되돌린다.
- 샘플 상품, 재고, 쿠폰 원본은 이름 기준으로 생성 또는 갱신한다.
- 다른 고객의 장바구니, 주문, 보유 쿠폰과 비샘플 상품/쿠폰 원본은 삭제하지 않는다.
- 초기화 후 프론트엔드는 `/products`로 이동하고 데모 고객 ID `1` 기준으로 상품, 장바구니, 쿠폰, 주문 조회 API를 다시 호출해 화면 상태를 갱신한다.
- 데모 초기화는 포트폴리오 시연 편의를 위한 보조 기능이며, 일반 고객 기능으로 취급하지 않는다.
